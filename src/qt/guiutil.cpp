...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("kolibricoin"))
        return false;
...
{
    // Convert kolibricoin:// to kolibricoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("kolibricoin://"))
    {
        uri.replace(0, 12, "kolibricoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KolibriCoin.lnk";
}
...
{
    return GetAutostartDir() / "kolibricoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("KolibriCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  kolibricoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("KolibriCoin-Qt"));
